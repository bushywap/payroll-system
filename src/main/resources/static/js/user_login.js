document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm.addEventListener('submit', async e => {
        ...
        try{
            ...
            const response = await fetch('/payroll-system/api/login', {
                method: 'POST',
			...
			if (data.success) {
			                 ...
			                 // REDIRECT UPDATED: Go to dashboard after login with context path
			                 setTimeout(() => {
			                     location.href = '/payroll-system/dashboard'; 
			                 }, 900);
			            } else {
    ...
    if (forgotLink && modalOverlay && cancelModalBtn && resetBtn && resetEmail) {
        ...
        resetBtn.addEventListener('click', async e => {
            ...
            try {
                // UPDATED: Added /payroll-system context path
                const response = await fetch('/payroll-system/api/forgot-password', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
